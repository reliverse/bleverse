...
function help() {
  console.log(`bleverse cli

Usage:
  bleverse registry list [registryPath]
  bleverse registry get <appId> [registryPath]
  bleverse registry rm <appId> [registryPath]
  bleverse registry upsert <json> [registryPath]

  bleverse app list [registryPath]
  bleverse app add <id> <repoDir> <workspacePath> <service> <smokeUrl> [registryPath]
  bleverse app init <id> <kind:web|api> <name> <repoDir> <service> <smokeUrl> [registryPath] [--template <github-url-or-shorthand>] [--template-branch <branch|sha>] [--template-recursive]
  bleverse app deploy <appId>
  bleverse app status <appId> [registryPath]
...
async function main() {
  ...
  if (scope === "app") {
    ...
    if (action === "init") {
      if (!a1 || !a2 || !a3 || !a4 || !a5 || !a6) {
        throw new Error(
          "usage: bleverse app init <id> <kind:web|api> <name> <repoDir> <service> <smokeUrl> [registryPath] [--template ...]",
        );
      }
  ...
  if (scope === "api" && action === "health") {
    ...
    const api = createBleverseApiClient({ baseUrl, token });
